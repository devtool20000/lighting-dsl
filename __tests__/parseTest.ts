...
describe("test parse tree", () => {
  test("single leaf", () => {
    const nodes = parse("name").children;
    expect(nodes[0].text).toBe("name");
    expect(nodes[0].isNested).toBe(false);
  });

  test("multiple leaf", () => {
    const nodes = parse("name age").children;
    expect(nodes[0].text).toBe("name");
    expect(nodes[0].isNested).toBe(false);
  ...
  test("single leaf with string", () => {
    const nodes = parse("name:`some:`").children;
  ...
  test("single nested children", () => {
    ...
    expect(nodes.children[0].text).toBe("a b c");
    expect(nodes.children[0].isNested).toBe(true);
  });
  ...
  test("leaf and nested children", () => {
    ...
    expect(nodes.children[0].text).toBe("a b c");
    expect(nodes.children[0].isNested).toBe(true);
    expect(nodes.children[1].text).toBe("name");
    expect(nodes.children[1].isNested).toBe(false);
  });

  test("leaf and nested children", () => {
    const nodes = parse("name {a b c}");

    expect(nodes.children[0].text).toBe("name");
    expect(nodes.children[0].isNested).toBe(false);
    expect(nodes.children[1].text).toBe("a b c");
    expect(nodes.children[1].isNested).toBe(true);
  });

  test("nested children with string", () => {
    const nodes = parse("{a b:`:some{` c}");

    expect(nodes.children[0].text).toBe("a b:`:some{` c");
    expect(nodes.children[0].isNested).toBe(true);
  });
  ...
  test("nested children with nested children", () => {
    ...
    expect(nodes.children[0].text).toBe("a b {nested a b c} c");
    expect(nodes.children[0].isNested).toBe(true);
  });
  ...
  test("nested children with nested children at end", () => {
    ...
    expect(nodes.children[0].text).toBe("a {n b}");
    expect(nodes.children[0].isNested).toBe(true);
  });
  ...
  test("nested children with nested x 2 children", () => {
    ...
    expect(nodes.children[0].text).toBe("a b {nested a b c {n2 a} d} c");
    expect(nodes.children[0].isNested).toBe(true);
  });
  ...
  describe("test parse node children", () => {
    test("access children",()=>{
      const nodes = parse("a b {nested a b c} c");
      const children = nodes.children[2]
      expect(children.children[0].text).toBe("nested")
    })
    ...
    //   expect(()=>{
    //     const nodes = parse("a b {nested a b c} c");
    //     const children = nodes.children[1]
